...
def generar_estrategia(diccionario_seller: dict, key: str, ) -> str:
    ...
    template = """
    Cuando respondas, comienza presentandote como  DAMELI, un asistente experto en e-commerce.
...
#datos_seller = consulta_db(config, key)   
...
async def chat_generate(request: Request, mensaje_usuario: str = Form(...)):
    ...
    # 3) devolvemos el template con los dos cuadros
    
    bot_message_html = markdown(
        generar_estrategia(datos_seller, key),
